 ...
 * ? optional match
 **/
export default (search, path, r) => {
  if (search.substr(search.length -1) === '/') search = search.toLowerCase().substr(0, search.length -1);
  if (path.substr(path.length -1) === '/') path = path.toLowerCase().substr(0, path.length -1);
  //quick exit if search is path
  if (search === path) {
    return true;
  }
  //so:
  ...
  for (let i = 1, so = 1; i < c; i++, so++) {
    if (typeof pathData[i] == 'undefined') {
      ...
      if (typeof searchData[so+1] == 'undefined') r.push(pathData[i]);
      else if (searchData[so+1] !== pathData[i]) r.push(pathData[i]);
      else {
        i--;
      }
    } else if (searchData[so] === '??') {
